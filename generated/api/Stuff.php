<?php
	...
	class Stuff extends MY_Controller
	{
		...
		protected $names_to_sort = array(
			'stuff_id', 'user_id', 'biz_id', 'password', 'role', 'level', 'time_create', 'time_delete', 'time_edit', 'creator_id', 'operator_id', 'status',
		...
		public function __construct()
		{
			...
			$this->names_to_return[] = 'stuff_id'; // 还有这里，OK，这就可以了
		...
		public function create()
		{
			...
			$this->form_validation->set_rules('biz_id', '所属商户ID', 'trim|required');
			$this->form_validation->set_rules('password', '员工操作密码', 'trim|required');
			$this->form_validation->set_rules('role', '角色', 'trim|');
			$this->form_validation->set_rules('level', '0暂不授权，1普通员工，10门店级，20品牌级，30企业级', 'trim|');
			$this->form_validation->set_rules('time_create', '创建时间', 'trim|');
			$this->form_validation->set_rules('time_delete', '删除时间', 'trim|required');
			$this->form_validation->set_rules('time_edit', '最后编辑时间', 'trim|');
			$this->form_validation->set_rules('creator_id', '创建者user_id', 'trim|required');
			$this->form_validation->set_rules('operator_id', '最后操作者user_id', 'trim|required');
